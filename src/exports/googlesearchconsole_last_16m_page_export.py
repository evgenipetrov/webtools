...
class GoogleSearchConsoleLast16mPageExport(BaseExportManager):
    ...
    def perform_pre_export_action(self):
        ...
        if self.project.gsc_auth_domain and self.project.gsc_property_name:
            use_existing = input(
                f"Use existing GSC settings? (Auth Domain: {self.project.gsc_auth_domain}, Property: {self.project.gsc_property_name}) [Y/n]: "
            )
    ...
    def perform_export(self):
        ...
        csv_file_path = os.path.join(self.export_folder, "gsc_last_16m_export.csv")
